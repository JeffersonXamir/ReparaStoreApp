...
namespace ReparaStoreApp.WPF.ViewModels.Productos
{
    public class ProductosViewModel : BaseViewModel
    {
        ...
        private readonly IDataService<ProductosItem> _ProductosDataService;
        private readonly IMapper _mapper;
        private readonly GenericListViewModel<ProductosItem> _ProductosListViewModel;
        public GenericListViewModel<ProductosItem> ProductosList => _ProductosListViewModel;

        private ProductosItem _currentProductos;
        public ProductosItem CurrentProductos
        {
            get => _currentProductos;
            set
            {
                _currentProductos = value;
                _editCopy = value != null ? (ProductosItem)CurrentProductos.Clone() : new ProductosItem();
        ...
        private ProductosItem _editCopy;
        public ProductosItem EditCopy
        {
            get => _editCopy;
            set
            {
        ...
        public ProductosViewModel(
                            ...
                            IDataService<ProductosItem> ProductosDataService,
                            ...
                            IMapper mapper) : base(eventAggregator)
        {
            ...
            _ProductosListViewModel = new GenericListViewModel<ProductosItem>(eventAggregator, ProductosDataService, mapper)
            {
                DisplayMemberPath = nameof(ProductosItem.Name),
                CodeMemberPath = nameof(ProductosItem.Code),
                PageSize = 15,
                SearchFunction = async (searchText) => await _ProductosDataService.SearchAsync(searchText, 1, 15),

                // Configurar el manejador de selección personalizado
                OnItemSelected = async (selectedItem) =>
                {
                    CurrentProductos = selectedItem;
                    await LoadProductosDetails(selectedItem.Id);
                }
            };
        }

        private async Task LoadProductosDetails(int ProductosId)
        {
            try
            {
                var ProductosDetails = await _ProductosDataService.GetByIdAsync(ProductosId);
        ...
        public override Task New()
        {
            try
            {
                ...
                _ProductosListViewModel.IsListEnabled = false;

                var settings = new Settings();
                int userId = settings.UserId;

                EditCopy = new ProductosItem(); // Crear una copia vacía para edición
                EditCopy.Id = 0;
        ...
        public override async Task Create()
        {
            try
            {
                ...
                if (CreationMode)
                {
                    ...
                    await _ProductosService.SaveProductosAsync(EditCopy);
                ...
                {
                    // Lógica para edición
                    await _ProductosService.UpdateProductosAsync(EditCopy);
                    CurrentProductos = EditCopy; // Actualizar el original
                ...
                CreationMode = false;
                EditMode = false;
                _ProductosListViewModel.IsListEnabled = true;
                await _ProductosListViewModel.LoadData();
                await base.Create();
            }
            catch (Exception ex)
            {
        ...
        public override async Task Activate()
        {
            try
            {
                await _ProductosService.ActivateProductosAsync(EditCopy);
                EditCopy.Activo = true;
                CurrentProductos.Activo = true;
        ...
        public override async Task Delete()
        {
            try
            {
                await _ProductosService.DeleteProductosAsync(EditCopy);
                EditCopy.Activo = false;
                CurrentProductos.Activo = false;
        ...
        public override Task Undo()
        {
            ...
            {
                EditCopy = (ProductosItem)CurrentProductos.Clone();
            }
